...
public class Queue {
    ...
    /**
     * This function converts every value in the queue to an integer, then
     * creates an int array and returns that
     * @return an int array based on all queue values
     */
    public int[] toIntArray()
    {
        int[] temp = new int[this.numElements];
        for(int i = 0; i < temp.length; i++)
        {
            temp[i] = Integer.valueOf(pop());
        }
